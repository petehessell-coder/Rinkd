...
export default function Scoresheet({ game, goals, penalties, shots, goalieChanges, isLeague = false, onClose }) {
  ...
  const homeTeam = isLeague ? (game.home_lt?.team?.name || game.home_lt?.team_name || "Home") : (game.home_team?.team_name || "Home");
  const awayTeam = isLeague ? (game.away_lt?.team?.name || game.away_lt?.team_name || "Away") : (game.away_team?.team_name || "Away");
