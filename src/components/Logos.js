...
import { C, colors as tokenColors } from '../lib/tokens';
  ...
  return (
    <svg viewBox="0 0 37 42" width={size} height={size} fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      ...
      {[
        ...
        <circle key={`on-${x}-${y}`} cx={x} cy={y} r="1.8" fill={C.red} filter={`url(#${glowId})`} />
      ))}
    </svg>
  );
}
...
export function LeaguesNavIcon({ size = 22 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      {/* Handles */}
      <path d="M5 6h2v2a2 2 0 002 2v1.5M19 6h-2v2a2 2 0 01-2 2v1.5" stroke={C.ice} strokeWidth="1.2" strokeLinecap="round" fill="none"/>
      {/* Cup body */}
      <path d="M7 4h10v5a5 5 0 01-10 0V4z" fill={C.red}/>
      {/* Cup highlight */}
      <path d="M9 4v3.5a3 3 0 001.5 2.6" stroke={C.ice} strokeOpacity="0.35" strokeWidth="1" strokeLinecap="round" fill="none"/>
      {/* Stem */}
      <rect x="10.5" y="13.5" width="3" height="3" fill={C.ice}/>
      {/* Base */}
      <rect x="7" y="17" width="10" height="3" rx="0.8" fill={C.ice}/>
    </svg>
  );
}
...
export function ProfileNavIcon({ size = 22 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      ...
      <circle cx="12" cy="12" r="11.2" fill="url(#rinkd-profile-bg)" stroke={C.red} strokeWidth="0.8"/>
      {/* Head */}
      <circle cx="12" cy="9.5" r="3.2" fill={C.ice}/>
      {/* Shoulders */}
      <path d="M4.5 22c1.4-3.7 4.6-5.5 7.5-5.5s6.1 1.8 7.5 5.5z" fill={C.ice}/>
    </svg>
  );
}
...
export function TournamentsNavIcon({ size = 22 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      ...
      <rect x="2" y="4"  width="5" height="3" rx="0.6" fill={C.navy} stroke={C.red} strokeWidth="1.1"/>
      <rect x="2" y="11" width="5" height="3" rx="0.6" fill={C.navy} stroke={C.red} strokeWidth="1.1"/>
      <rect x="2" y="17" width="5" height="3" rx="0.6" fill={C.navy} stroke={C.red} strokeWidth="1.1"/>
      {/* Connectors left → semifinal */}
      <path d="M7 5.5h2v6.5M7 12.5h2M7 18.5h2v-6" stroke={C.ice} strokeWidth="1.1" strokeLinecap="round" fill="none"/>
      {/* Semifinal box */}
      <rect x="9" y="9" width="5" height="6" rx="0.6" fill={C.red}/>
      {/* Connector → final */}
      <path d="M14 12h3" stroke={C.ice} strokeWidth="1.1" strokeLinecap="round" fill="none"/>
      {/* Trophy/winner */}
      <circle cx="19" cy="12" r="2.6" fill={C.ice}/>
    </svg>
  );
}
...
export function ChirpNavIcon({ size = 22, inline = true }) {
  if (!inline) {
    ...
    return (
      <svg width={size} height={size} viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
        {/* Upper jaw */}
        <path d="M2 11 L17 4 L17 10 Z" fill={C.red}/>
        {/* Lower jaw (open) */}
        <path d="M2 11 L17 18 L17 12 Z" fill={C.red}/>
        {/* Eye — ice white with navy pupil */}
        <circle cx="6.5" cy="8.8" r="1.7" fill={C.ice}/>
        <circle cx="6.5" cy="8.8" r="0.75" fill={C.navy}/>
      </svg>
    );
  }
  ...
  return (
    <svg width={w} height={size} viewBox="0 0 56 22" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      ...
      <path d="M0 11 L13 5 L13 10 Z" fill={C.red}/>
      {/* Lower jaw of the beak — opened to let the word fly out */}
      <path d="M0 11 L13 17 L13 12 Z" fill={C.red}/>
      {/* Eye — ice white with navy pupil. Sits on the upper jaw. */}
      <circle cx="4" cy="9" r="1.4" fill={C.ice}/>
      <circle cx="4" cy="9" r="0.6" fill={C.navy}/>
...
export function TierBadge({ tier, size = 'sm' }) {
  const colors = { Mite:C.steel,Squirt:tokenColors.success,Peewee:'#0EA5E9',Bantam:tokenColors.warning,Midget:tokenColors.premium,Junior:C.red,Pro:C.ice };
  const color = colors[tier]||C.steel;
...
export function TeamLogo({ team, size = 40, radius, style }) {
  ...
  const color = team?.logo_color || C.blue;
...
export function Avatar({ profile, size = 36 }) {
  ...
  return (
    <div style={{
      ...common,
      background: profile?.avatar_color || C.blue,
