...
//
// Magic-link landing for team-manager invites. Three states:
//   1. Not signed in → redirect to /login?returnTo=/accept-team-invite?token=...
...
export default function AcceptTeamInvite({ profile }) {
  ...
  useEffect(() => {
    ...
    if (!user) {
      // Bounce to login + come right back. Wrap the whole accept URL in
      // returnTo so the token survives the round trip.
      const back = encodeURIComponent(`/accept-team-invite?token=${encodeURIComponent(token)}`);
    ...
    (async () => {
      try {
        const { leagueId, teamId } = await acceptTeamManagerInvite(token);
        if (cancelled) return;
        setState({ kind: 'success', leagueId, teamId });
      ...
      <Layout profile={profile}>
        <div style={center}>
          <div style={{ maxWidth: 360 }}>
            ...
            <div style={{ fontSize: 13, color: 'rgba(244,247,250,0.55)' }}>One moment while we set you up as a team manager.</div>
          </div>
        </div>
      </Layout>
    );
      ...
      <Layout profile={profile}>
        <div style={center}>
          <div style={{ maxWidth: 360 }}>
            ...
            <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontStyle: 'italic', fontWeight: 900, fontSize: 22, marginBottom: 8 }}>You're a team manager.</div>
