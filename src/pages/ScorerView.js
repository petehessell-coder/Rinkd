...
function SecLabel({ children, style = {} }) {
  return <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', color: 'rgba(244,247,250,0.3)', textTransform: 'uppercase', marginBottom: 8, marginTop: 4, ...style }}>{children}</div>;
}

function Card({ children, style = {} }) {
  return <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: '14px 16px', marginBottom: 16, ...style }}>{children}</div>;
}

function AddBtn({ onClick, children }) {
  return (
    <button onClick={onClick}
      style={{ width: '100%', padding: 10, background: 'rgba(46,91,140,0.15)', border: '0.5px dashed rgba(46,91,140,0.5)', borderRadius: 8, color: C.ice, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Barlow, sans-serif', marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: 0.8, transition: 'all 0.15s' }}
      onMouseEnter={e => { e.currentTarget.style.background = C.ice; e.currentTarget.style.color = C.navy; e.currentTarget.style.opacity = '1'; e.currentTarget.style.borderStyle = 'solid'; }}
      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(46,91,140,0.15)'; e.currentTarget.style.color = C.ice; e.currentTarget.style.opacity = '0.8'; e.currentTarget.style.borderStyle = 'dashed'; }}>
      {children}
    </button>
  );
}
      ...
      <div style={{ background: C.navy, borderRadius: '16px 16px 0 0', padding: 20, width: '100%', maxWidth: 480, borderTop: `0.5px solid ${C.border}` }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <button onClick={onClose}
            ...
            onMouseEnter={e => { e.currentTarget.style.background = C.ice; e.currentTarget.style.color = C.navy; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(244,247,250,0.08)'; e.currentTarget.style.color = C.ice; }}>
            Cancel
          </button>
...
export default function ScorerView() {
  ...
  const [goalieModal, setGoalieModal] = useState(null); // team_id
  ...
  const load = useCallback(async () => {
    ...
    setPenalties(pl || []);
    const shotMap = {};
    (sl || []).forEach(s => { if (!shotMap[s.team_id]) shotMap[s.team_id] = 0; shotMap[s.team_id] += s.count; });
    setShots(shotMap);
    setGoalieChanges(gc || []);
    setLoading(false);
  }, [gameId, navigate]);
  ...
  // ── SCORE UPDATE ─────────────────────────────────────────
  const updateScore = async (newHome, newAway, newPeriod, newStatus) => {
    setSaving(true);
    await supabase.from('games').update({
      home_score: newHome, away_score: newAway,
      period: newPeriod, status: newStatus,
    }).eq('id', gameId);
    setSaving(false);
  };
  ...
  const changeScore = (team, delta) => {
    ...
    updateScore(hs, as, period, status === 'scheduled' ? 'live' : status);
  ...
  const saveGoal = async () => {
    ...
    const { data } = await supabase.from('game_goals').insert({
      ...
      time_in_period: goalForm.time_in_period || null,
  ...
  const savePenalty = async () => {
    ...
    const duration = PENALTY_DURATIONS[penaltyForm.severity] || 2;
    const { data } = await supabase.from('game_penalties').insert({
      ...
      time_in_period: penaltyForm.time_in_period || null,
    }).select().single();
  ...
  const saveGoalie = async () => {
    ...
    const { data } = await supabase.from('game_goalie_changes').insert({
      ...
      period: goalieForm.period,
      time_in_period: goalieForm.time_in_period || null,
    }).select().single();
  ...
  if (loading) return (
    <div style={{ background: C.dark, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.ice, fontFamily: 'Barlow, sans-serif' }}>
      Loading game...
    </div>
  );
  ...
  const severityColor = (s) => s.includes('Major') || s.includes('Match') ? C.red : s.includes('Double') ? '#F59E0B' : '#F59E0B';
    ...
    <div style={{ background: C.dark, minHeight: '100vh', fontFamily: 'Barlow, sans-serif', color: C.ice, maxWidth: 480, margin: '0 auto', paddingBottom: 40 }}>
      ...
      <div style={{ background: C.navy, padding: '14px 16px', borderBottom: `0.5px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ textAlign: 'center', flex: 1 }}>
          <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontStyle: 'italic', fontWeight: 900, fontSize: 15, color: C.ice }}>
            {homeTeam?.team_name} vs {awayTeam?.team_name}
          </div>
          <div style={{ fontSize: 11, color: 'rgba(244,247,250,0.4)', marginTop: 2 }}>
            {game.rink?.sub_rink} · {game.tournament?.name}
          </div>
        </div>
      ...
      <div style={{ padding: 16 }}>
          ...
          {[[homeTeam, homeScore, 'home'], [awayTeam, awayScore, 'away']].map(([team, score, side], i) => (
            <div key={side} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderTop: i > 0 ? `0.5px solid rgba(244,247,250,0.07)` : 'none', marginTop: i > 0 ? 6 : 0, paddingTop: i > 0 ? 12 : 6 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontStyle: 'italic', fontWeight: 900, fontSize: 44, color: C.ice, width: 56, textAlign: 'center', lineHeight: 1 }}>{score}</div>
        ...
        <Card>
          {[homeTeam, awayTeam].map((team, i) => (
            <div key={team?.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderTop: i > 0 ? `0.5px solid rgba(244,247,250,0.07)` : 'none' }}>
              ...
              <button onClick={() => changeShots(team?.id, -1)} style={{ width: 36, height: 36, background: 'rgba(244,247,250,0.08)', border: 'none', borderRadius: 8, color: C.ice, fontSize: 18, cursor: 'pointer', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = C.ice; e.currentTarget.style.color = C.navy; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(244,247,250,0.08)'; e.currentTarget.style.color = C.ice; }}>−</button>
              <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontStyle: 'italic', fontWeight: 900, fontSize: 32, color: C.ice, width: 48, textAlign: 'center' }}>{shots[team?.id] || 0}</span>
              <button onClick={() => changeShots(team?.id, 1)} style={{ width: 44, height: 44, background: C.blue, border: 'none', borderRadius: 8, color: C.ice, fontSize: 22, cursor: 'pointer', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = C.ice; e.currentTarget.style.color = C.navy; }}
        ...
        <Card>
          ...
          {goals.map(g => (
            <div key={g.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 0', borderBottom: `0.5px solid rgba(244,247,250,0.06)` }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', background: g.team_id === homeTeam?.id ? '#1a4a7a' : '#6b1520', flexShrink: 0, marginTop: 4 }} />
              <div style={{ flex: 1 }}>
              ...
              <button onClick={() => deleteGoal(g.id)} style={{ background: 'none', border: 'none', color: 'rgba(244,247,250,0.2)', cursor: 'pointer', fontSize: 14, padding: '0 2px' }}
        ...
        <Card>
          {[homeTeam, awayTeam].map((team, ti) => {
            const changes = goalieChanges.filter(g => g.team_id === team?.id);
            return (
              <div key={team?.id} style={{ marginBottom: ti === 0 ? 12 : 0 }}>
                ...
                {changes.map(c => (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: `0.5px solid rgba(244,247,250,0.06)` }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: C.ice }}>
                        {c.goalie_out_number ? `#${c.goalie_out_number}` : '?'} → {c.goalie_in_number ? `#${c.goalie_in_number}` : '?'}
                      </div>
                      <div style={{ fontSize: 11, color: 'rgba(244,247,250,0.4)', marginTop: 2 }}>
                ...
                <AddBtn onClick={() => { setGoalieForm({ goalie_out_number: '', goalie_in_number: '', period, time_in_period: '' }); setGoalieModal(team?.id); }}>
                  + Log Change — {team?.team_name}
                </AddBtn>
              </div>
            );
          })}
        ...
        {status === 'final' && (
          <div style={{ textAlign: 'center', padding: 14, background: 'rgba(215,38,56,0.1)', border: `0.5px solid rgba(215,38,56,0.3)`, borderRadius: 999, fontSize: 14, fontWeight: 700, color: C.red }}>
            ✓ Game Finalized — Standings Updated
          </div>
        )}
