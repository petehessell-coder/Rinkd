...
export default function ScorerView() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: g, error: gErr } = isLeague
      ...
      : await supabase.from('games')
          ...
          .select('*, home_team:tournament_teams!home_team_id(id,team_name), away_team:tournament_teams!away_team_id(id,team_name), rink:rinks(name,sub_rink), tournament:tournaments(name,director_id,settings,is_activated,usah_compliant_scoresheet,usah_association_name,usah_classification,division_label,scoring_source)')
          .eq('id', gameId).single();
  ...
  // SOCIAL-2 external-scoring gate. GameSheet-synced tournaments take scores from
  // the poller — manual scoring would create a conflicting second source, so the
