...
### `ENRICH-1` + `ONBOARD-1` — User profile enrichment + signup streamline (spec'd May 28, 2026 — reconciled w/ REG-2)
...
**Priority + sequencing:** Both **P2**, **no pilot dependency.** Effort: **`ENRICH-1` ~1–2 days** (single `user_profile_enrichment` migration + gated `get_user_email` RPC + bounded `last_seen_at` wiring + extending the `profiles_guard_privileged` trigger to lock `date_of_birth`). **`ONBOARD-1` ~3–5 days** (Auth.js single-step strip + OnboardingModal extension + dismissible `/feed` banner + 6 files touched). **`ONBOARD-1` depends on `ENRICH-1`** — the form writes to columns the migration adds. Both are **orthogonal to REG-2 Phase 1** (identity spine + `auth_user_id` decouple): REG-2 Phase 1 expects these adult-self-signup columns to already exist and will only add `auth_user_id` / `account_type` / household / consent. **Build order:** `ENRICH-1` → `ONBOARD-1` → REG-2 Phase 1; **`MONEY-1` is fully independent** and can ship in any slot.
