...
- **`SCORER-TAP-1` — tap a team / score to log a goal** (`c54ff751`, merged `d932416c`). From the Jun 1 Nick (BLPA) call — timekeepers are 10–70 yrs old, wants goal entry stupid-proof. In `src/pages/ScorerView.js` (shared component → tournament **and** league at once) the team name (now carrying a "+ Goal" pill) and the big score number are tap targets that open the Log Goal form prefilled to that side. Non-destructive: opening writes nothing, Save confirms, ✕ undoes; +/− stays as the score-correction escape hatch (the `changePeriod` mismatch guard relies on it). Pure client, no schema/RLS. ⚠️ Still wants a real-iPad eyeball (PRE-5) — confirm the tap targets feel right on hardware.
