...
- **S3 next (the remaining piece — makes it usable without SQL, à la M4 for divisions):** a director-facing **GameSheet tab** in `TournamentManage` — create/edit/remove a link (season id + optional division → sets `scoring_source='external'`), show `last_synced_at`/`last_sync_note`, and a **pending-matches list** with Confirm/Ignore (direct RLS writes; cron does the polling). Optional "Sync now" deferred.
