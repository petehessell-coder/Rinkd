...
## 4. Current state — verified May 18, 2026 late afternoon

`origin/main` HEAD is **`45f71a6d`** (`feat: Cloudflare Turnstile on signup form — bot protection`). Vercel deployed at build ID `45f71a6dd568`. Everything we did today across morning + afternoon + late afternoon sessions is live in production:
- Forgot Password config
- VAPID rotation + Edge Function deploy
- push.js InvalidStateError fix
- Tournament-scoped feed (recaps + composer)
- Auto-follow-Pete trigger + 19-user backfill
- Multi-director support
- 4 SECURITY DEFINER views flipped to invoker; media bucket listing closed
- Turnstile bot protection on signup

Recent history on `origin/main`:

```
45f71a6d  feat: Cloudflare Turnstile on signup form — bot protection ← HEAD, deployed
...
- Multi-director: ✅ live — Scorers tab → Directors section. Founder badge protects the original director. RLS uses `is_tournament_director()` helper.
- Turnstile bot protection: ✅ live on signup, login, password reset (Supabase's CAPTCHA Protection covers all auth endpoints). Direct API signup without a token returns `400 captcha_failed`. Bug report + survey form gating filed as a post-pilot follow-up.
- Security advisor pass (May 18 late afternoon): 4 SECURITY DEFINER views fixed, media bucket listing closed.
- 7 Edge Functions deployed (`send-invite`, `submit-scoresheet`, `send-push`, `schedule-ics`, `send-game-reminders`, `send-onboarding-emails`, `delete-account`, `send-recap-push`) — last is the pilot push pipeline, others are pre-existing infra.
