...
- **Still open on SOCIAL-2:** (a) **GO-LIVE OPS** — set `CRON_KEY` + schedule pg_cron (above); until then the poller only runs on manual invoke. (b) **Per-division linking** — `gamesheet_links.division_id` + the poller already support it, but the S3 link form is event-wide only (the division picker needs the multi-division UI, which lands at M7 merge). (c) **SOCIAL-1 CSV import** — the manual-import alternative to the poller — is **BLOCKED on a real GameSheet CSV export sample** (don't guess the column mapping → silent-wrong-data risk; the poller already covers the automated path). (d) This branch is off `main` and **mergeable independently of multi-division** (additive, pilot-safe — coordinate the eventual merge so it doesn't precede a pilot-risky change).
