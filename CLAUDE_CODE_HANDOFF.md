...
- **`SCORER-TAP-1` — tap team/score to log a goal (✅ DONE on branch, pending iPad test + merge).** Nick's timekeepers are 10–70 yrs old; he wants goal entry stupid-proof. Tapping a team name (now carries a "+ Goal" affordance) or its big score opens the Log Goal form prefilled to that side. `src/pages/ScorerView.js` (shared component → ships tournament **and** league at once). Non-destructive: opening writes nothing, Save is the confirm, ✕ undoes; +/− stays as the score-correction escape hatch (the `changePeriod` mismatch guard depends on it). Branch `feature/scorer-tap-to-log` (`c54ff751`), Vercel preview alias `rinkd-git-feature-scorer-tap-to-log-petehessell-coders-projects.vercel.app`. Possible fast-follow: a 1-tap "who scored? #__" quick mode if the full form is too much for the youngest timekeepers. **Do NOT merge until a real iPad eyeball (pairs with PRE-5).**
