...
## 4. Current state — verified May 18, 2026 afternoon

`origin/main` HEAD is **`ae4d7985`** (`feat: tournament feed composer — text + photo, with report/block`). Vercel deployed at build ID `ae4d79852ca5`. Everything we did today (forgot password config, VAPID rotation + Edge Function deploy, push.js InvalidStateError fix, tournament-scoped feed, tournament feed composer, auto-follow-Pete trigger + 19-user backfill) is live in production.

Recent history on `origin/main`:

```
ae4d7985  feat: tournament feed composer — text + photo, with report/block ← HEAD, deployed
...
**Operational state (verified May 18 afternoon):**
...
2. Run `cd ~/Downloads/rinkd_live && git log --oneline -10 && git status` to confirm state matches §4.
   - Expected `origin/main` HEAD: **`ae4d7985`** (`feat: tournament feed composer …`). If it's later, even better — read the new commits to catch up.
   ...
   - Confirm the new DB shape (May 18 schema): `select column_name from information_schema.columns where table_schema='public' and table_name='posts' and column_name='tournament_id'` should return one row. `select tgname from pg_trigger where tgname = 'tr_auto_follow_pete'` should return one row.
