...
**What's on the BRANCH** (`feature/multi-division`, commit `04fa1ebb`, pushed; Vercel preview alias `rinkd-git-feature-multi-division-...vercel.app`):
...
**⭐ The M7 cutover gotcha:** the branch reads `tournament_standings_md`. **At M7 (post-Jun-14):** (1) `DROP VIEW tournament_standings; ALTER VIEW tournament_standings_md RENAME TO tournament_standings;` (2) flip the `STANDINGS_VIEW` constant in `Tournament.js` back to `'tournament_standings'`; (3) merge the branch to main. Also re-point `src/lib/tournamentManage.js` (3 `tournament_standings` queries) to the rebuilt view if division-scoped standings are needed there.

**Sandbox test event (safe to delete):** `4f06485a-b090-4031-8182-eaa68cf00fc1` — "SANDBOX — Multi-Division Test", 2 divisions (10U A / 12U AA), 4 teams + 3 final games each. Used to verify M3's switcher data layer (two correct, separate standings tables confirmed). **A new session may `DELETE FROM tournaments WHERE id='4f06485a-...'` to clean up** (cascades to divisions/teams/games).

**Remaining milestones:**
- **M4** — `TournamentManage.js` (1594 lines): a **Divisions tab** so directors can create/edit/reorder divisions (today divisions only exist via SQL). The blocker for real-world multi-division use.
- **M5** — `TournamentCreate` optional Divisions step + make `generateChampionshipBracket` division-scoped.
