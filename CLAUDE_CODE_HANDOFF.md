...
**TIER 1 — build/fix before pilot:** ~~wire Sentry DSN~~ (already live since May 12 — see finding above) · **bot filtering in analytics (DONE May 25 — `analytics.js` `isLikelyBot()` drops crawler/headless/`navigator.webdriver` traffic; ~7% of sessions were bots skewing the funnel)** · verify activation-gate RPCs fail-closed for anon (advisor flagged SECURITY-DEFINER funcs anon-executable; confirm `admin_set_activation` etc. reject anon — protects the moat) · enable Supabase leaked-password protection (toggle) · `login_failed` sanity (31 fails/7 sessions/0 forgot-clicks — confirm internal-testing vs a real login bug). **Note:** the auth funnel itself is **already fully instrumented** (`auth_view`/`auth_first_input`/`login_failed`/`signup_step_advanced`/`signup_success`/`forgot_password_clicked`/`password_reset_requested` all wired in `Auth.js`) — the only real gap was bot filtering. The thin/low-confidence funnel data is volume + age (events born May 20), not missing events.
