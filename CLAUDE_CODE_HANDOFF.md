...
- ⚠️ **Sentry is coded but DARK in prod.** `@sentry/react` is wired (`src/lib/sentry.js`, ErrorBoundary, `initSentry()` in `index.js`) but stays dormant unless `REACT_APP_SENTRY_DSN` is set in Vercel — and the `rinkd-llc` Sentry org has **zero issues ever**. So the DSN isn't set → no client-crash visibility during the live event. **Fix: set `REACT_APP_SENTRY_DSN` in Vercel (Sentry → Settings → Client Keys/DSN) + redeploy + fire a test throw.** ~5 min, high value.
...
**TIER 1 — build/fix before pilot:** ⭐ wire Sentry DSN (above) · **bot filtering in analytics (DONE May 25 — `analytics.js` `isLikelyBot()` drops crawler/headless/`navigator.webdriver` traffic; ~7% of sessions were bots skewing the funnel)** · verify activation-gate RPCs fail-closed for anon (advisor flagged SECURITY-DEFINER funcs anon-executable; confirm `admin_set_activation` etc. reject anon — protects the moat) · enable Supabase leaked-password protection (toggle) · `login_failed` sanity (31 fails/7 sessions/0 forgot-clicks — confirm internal-testing vs a real login bug). **Note:** the auth funnel itself is **already fully instrumented** (`auth_view`/`auth_first_input`/`login_failed`/`signup_step_advanced`/`signup_success`/`forgot_password_clicked`/`password_reset_requested` all wired in `Auth.js`) — the only real gap was bot filtering. The thin/low-confidence funnel data is volume + age (events born May 20), not missing events.

**TIER 2 — quality if time:** onboarding step-2 "suggested follows" still shows **demo accounts** (pilot will drive new signups who see this — ~30 min) · `/pricing` + tape-font real-screen eyeball · public tournament landing copy pass.
